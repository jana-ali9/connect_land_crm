        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($contract->property_type === 'building' && $contract->building)
                        <div class="col-md-6">
                            <h6 class="fw-normal text-muted">Unit</h6>
                            <h6 class="fs-16">{{ $contract->building->name }}</h6>
                            <address>
                                {{ $contract->building->description }}
                                <br>
                                {{ $contract->building->address }}
                            </address>
                        </div>
                    @elseif ($contract->property_type === 'land' && $contract->land)
                        <div class="col-md-6">
                            <h6 class="fw-normal text-muted">Land</h6>
                            <h6 class="fs-16">{{ $contract->land->name }}</h6>
                            <address>
                                {{ $contract->land->description }}
                                <br>
                                {{ $contract->land->location }}
                            </address>
                        </div>
                    ...
                    <!-- end row -->
                    <h4 id="scrollspyHeading1">Purchase price : {{ $contract->base_rent }}$</h4>
                                    ...
                                    <thead class="bg-light bg-opacity-50">
                                        <tr>
                                            ...
                                            <th class="text-end border-0 py-2"><a href="#" id="add-service-btn"
                                                    ...
                                                    data-bs-target="#addServiceModal">Add Service</a><a href="#"
                                                    id="add-service-btn" class="btn btn-sm btn-soft-secondary"
                                                    data-bs-toggle="modal" data-bs-target="#adddateServiceModal">Add Date
                                                    Service</a>
                    ...
                    <!-- Modal لإضافة خدمة جديدة -->
                    <div class="modal fade" id="adddateServiceModal" tabindex="-1"
                        aria-labelledby="adddateServiceModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="adddateServiceModalLabel">Add Date Service</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>

                                <!-- ✅ Form يجب أن يحتوي على method و action -->
                                <form method="POST" action="{{ route('addDate-services.store') }}">
                                    @csrf
                                    <div class="modal-body">
                                        <input type="hidden" name="contract_id" value="{{ $contract->id }}">
                                        <div class="mt-3">
                                            <label for="end_date" class="form-label">Custom Price</label>
                                            <input type="date" id="end_date" name="end_date" class="form-control"
                                                min="{{ $contract->end_date ?? 0 }}">
                                        </div>
                                    </div>
                                    ...
                                    <table class="table mb-0">
                                        <thead class="bg-light bg-opacity-50">
                                            <tr>
                                                ...
                                                <th class="text-end border-0 py-2" scope="col">Action <a
                                                        href="#" id="add-service-btn"
                                                        class="btn btn-sm btn-soft-primary" data-bs-toggle="modal"
                                                        data-bs-target="#payContractAmount">
                                                        <!-- ✅ أضف هذا الجزء -->
                                                        Pay invoices
                                                    </a></th>
                                            </tr>
